# NGINX + PHP-FPM Docker image for Laravel
...
docker run -d \
    ...
    -v ~/.ssh:/root/.ssh \
    abrahambosch/nginx-php-fpm-laravel(:<version-tag>)
```
...
services:
    php-fpm:
        image: abrahambosch/nginx-php-fpm-laravel(:<version-tag>)
        environment:
            XDEBUG: true
            PHP_IDE_CONFIG: "serverName=phpstorm-server" #This is required for PhpStorm only for path mappings
...
## local testing
```
run -it -v ~/src/webapp:/var/www/html -e "NGINX_PORT=80" -e "NGINX_PROXY_HOST=127.0.0.1" -e "NGINX_PROXY_PORT=9000"  abrahambosch/nginx-php-fpm-laravel:latest bash
```
